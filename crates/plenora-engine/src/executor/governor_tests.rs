...
fn tee_fan_out_counts_quota_once_and_releases_after_last_reader() {
    ...
    assert_eq!(governor.reserved_bytes(), bytes, "accounting unico al fan-out");
...
fn streaming_chain_propagates_sequence_one_to_one() {
    ...
    assert_eq!(governed.len(), 2, "un batch in uscita per batch in ingresso");
    for (index, batch) in governed.iter().enumerate() {
...
fn blocking_segment_reassigns_sequence_deterministically() {
    ...
    let blocking_seq = || {
        ...
        assert!(node.bytes_in > 0 && node.bytes_out > 0, "metriche byte popolate");
...
fn memory_metrics_are_populated_and_leak_free_after_drain() {
    ...
    assert!(metrics.memory.budget_bytes > 0, "budget dai limiti effettivi");
