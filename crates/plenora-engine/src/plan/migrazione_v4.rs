...
//!
//! La migrazione tocca **due** cose: `schema_version`, che diventa 5, e il
//! blocco `limits`, che attraversa le due strutture tipizzate. Tutto il
//! resto del piano — nodi, input, `crs_decisions`, `output` — passa
//! invariato sull'albero JSON: non viene ne' riordinato ne' normalizzato,
//! perche' normalizzare e' compito della canonicalizzazione e farlo in due
//! posti significa poterlo fare in due modi.
//!
//! Ne segue che un piano v4 e il v5 equivalente producono lo **stesso** piano
//! canonico e lo **stesso** `plan_hash` (verificato dai test).
//!
//! L'idempotenza e' quella di [`testo_canonico_v5`], che e' l'ingresso reale:
//! applicato al proprio risultato lo lascia invariato, perche' il risultato
//! dichiara la v5 e la v5 attraversa senza copia. [`migra_v4_a_v5`] da sola
//! rifiuta un piano gia' migrato invece di operarci in silenzio: chiamarla
//! due volte e' un errore di chi chiama, e un errore di chi chiama va detto.
...
pub fn versione_dichiarata(json_text: &str) -> Result<u16> {
...
/// `schema_version: 4`, se non e' un oggetto, o se il blocco `limits` non e'
/// un oggetto valido per la v4 — chiavi sconosciute e il nome della v5
/// incluse. `PlenoraError::Internal` solo per un fallimento di
/// serializzazione, che su una struttura di `Option<numero>` non ha modo di
/// accadere: e' il caso "impossibile" reso esplicito invece che assunto (R6).
pub fn migra_v4_a_v5(json_text: &str) -> Result<String> {
    ...
    if let Some(limiti) = oggetto.get("limits") {
        let v4: LimitsOverrideV4 =
            serde_json::from_value(limiti.clone()).map_err(|errore_serde| {
                errore(format!(
                    "limiti non validi per un piano v4: {errore_serde}. \
...
///
/// Il tetto `max_plan_json_bytes` si applica al testo **fornito**, prima di
/// costruire qualunque albero JSON (ADR 6): migrare significa allocare, e
/// allocare guidati dal contenuto prima di averlo limitato e' esattamente
/// cio' che ADR 6 vieta. Il testo migrato viene poi ricontrollato da
/// [`super::PlanV5::parse`]; non c'e' divergenza fra un v4 e il v5
/// equivalente, perche' il v5 equivalente ha la stessa chiave lunga e quindi
/// la stessa dimensione.
///
/// # Errors
///
/// `PlenoraError::DataMapping` se il testo non e' JSON valido;
/// `PlenoraError::InvalidPlan` se il testo supera `max_plan_json_bytes`, se
/// contiene chiavi duplicate, se non dichiara una `schema_version`
/// leggibile, se dichiara una versione `<= 3` (quei piani sono la forma
/// lineare legacy: passano da [`super::PlanV5::from_legacy`], che li porta al
/// canonico v5 senza toccare la v4) o una versione futura, o se la migrazione
/// v4 fallisce.
pub fn testo_canonico_v5<'a>(json_text: &'a str, plan_limits: &PlanLimits) -> Result<Cow<'a, str>> {
    ...
    match versione_dichiarata(json_text)? {
        ...
        PLAN_SCHEMA_VERSION_V4 => Ok(Cow::Owned(migra_v4_a_v5(json_text)?)),
