...
| Campo | Tipo | Default | Vincoli |
|---|---|---|---|
| `column` | `string` | obbligatorio | colonna esistente, numerica; **non `Utf8`** per `rank`, `dense_rank`, `percent_rank`, `cume_dist` |
...
**Input:** tabella unaria; `column` numerica, `group_by`/`order_column` esistenti se specificati. Le quattro funzioni di **rango** (`rank`, `dense_rank`, `percent_rank`, `cume_dist`) ordinano, quindi non convertono e non accettano una `column` `Utf8`: il testo numerico non ha un ordine esatto (vedi `errori-e-limiti.md`, «Le funzioni di rango non accettano colonne testuali»). Le altre varianti la accettano.
