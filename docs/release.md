...
| `geometry_diagnostics` rende errore invece di un referto | quando la validazione OGC non conclude, la funzione rende `ExtendedAlgorithmError::ValidazioneNonConclusa` invece di un `GeometryDiagnostics` con `is_valid: false`. Chi la chiamava su ingressi ostili riceve ora un `Err` dove prima otteneva un referto: quel referto **affermava invalida** una geometria su cui il validatore si era interrotto. La funzione continua ad accettare la topologia invalida — è il suo mestiere — ma non un verdetto che nessuno ha emesso |
