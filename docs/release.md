...
| `EvidenzaDiLimite` è un tipo pubblico | accompagna `UnattributedMemoryPressure` con cinque contatori, quattro dei quali `Option<u64>` perché **`None` non è zero**: un contatore non osservabile non deve poter essere letto come «nessun evento». È un tipo nuovo, quindi non rompe nulla oggi, ma i suoi campi sono pubblici e cambiarli lo farà |
